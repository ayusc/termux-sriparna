...
def voice_assistant(text):
    ...
    if any(
        ...
        battery_status_output = subprocess.check_output(['termux-battery-status']).decode(
            'utf-8'
        )
    ...
    if any(
        ...
        try:
            ...
            gps_output = subprocess.check_output(['termux-location', '-p', 'gps']).decode(
                'utf-8'
            )
    ...
    stream = client.chat.completions.create(
        model='gpt-4-turbo', messages=[{'role': 'user', 'content': text}], stream=True,
    )
    typed_text = ""
    for chunk in stream:
